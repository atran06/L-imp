...
def scan(line):
    ...
    token = ""
    for char in line: # goes through each character in the string and adds the char into the variable 'token' until one of the conditions are met
        if char == " ":
            if token != "": 
            ...
            if token != "": 
                ...
                token = ""
            basicTokens.append(char) # appends the punctuation
        else: token += char
